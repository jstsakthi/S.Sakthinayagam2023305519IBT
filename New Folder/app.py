...
st.markdown("<h1 style='text-align: center; color: #f2c40a;'> BRAIN TUMOR SEGMENTATION APP </h1>", unsafe_allow_html=True)
st.markdown("---")
...
if uploaded_file is not None:
    ...
    # Display results
    st.markdown("<h2 style='color:#1a28bd;'> SEGMENTATION RESULT </h2>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)

    with col1:
        st.image(img_resized, caption="Original MRI IMAGE", use_container_width=True)
    with col2:
        st.image(prediction, caption="Predicted MASK ", use_container_width=True, clamp=True)
